...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'menu', fromMe: true, desc: Lang.ALIVE_DESC}, (async (message, match) => {
   ...
   Asena.addCommand({pattern: 'menu', fromMe: false, desc: Lang.ALIVE_DESC}, (async (message, match) => {
        
        let pp
        try { pp = await message.client.getProfilePicture(message.jid.includes('-') ? message.data.participant : message.jid ); } catch { pp = await message.client.getProfilePicture(); }
        await axios.get(Config.LOGOSK, {responseType: 'arraybuffer'}).then(async (res) => { await message.client.sendMessage(message.jid, res.data, MessageType.image, { caption: © ʜᴇͧʀᷤᴍͤɪͬ† ⚚ ᴘɪⷡᴋⷪᴀⷮᴄᷤʜᴜ  }); });
    }));
