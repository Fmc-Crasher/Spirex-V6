...
if (res.links[0].url.includes('mp4')) return await msg.client.sendMessage(msg.jid, buffer, MessageType.video, { mimetype: Mimetype.mp4, caption: 'ᴛɪᴛʟᴇ: ' + `${res.caption}` + '\n\n ᴜѕᴇʀɴᴀᴍᴇ: *' + `${res.username}` + '*', quoted: msg.data});
if (res.links[0].url.includes('jpg')) return await msg.client.sendMessage(msg.jid, buffer, MessageType.image, { mimetype: Mimetype.jpg, caption: 'ᴛɪᴛʟᴇ: ' + `${res.caption}` + '\n\n ᴜѕᴇʀɴᴀᴍᴇ *' + `${res.username}` + '*', quoted: msg.data});
}
